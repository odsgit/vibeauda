...
import type { Part, MidiNote, SheetData } from '../types/sheet';
...
/**
 * Renders `data`/`part` as VexFlow SVG notation into `container` (any DOM
 * element, mounted or detached). Shared by the live SheetView component and
 * the PDF export path so both stay in sync with a single layout algorithm.
 */
export function renderSheetSvg(container: HTMLDivElement, data: SheetData, part: Part): void {
